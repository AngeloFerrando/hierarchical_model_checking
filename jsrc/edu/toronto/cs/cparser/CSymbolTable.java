...
public class CSymbolTable {
  ...
  public TNode add(String name, TNode node) {
    assert (node.getType () != 0);
    // System.err.println ("+++ Adding to symtable: " + name + ", " + node + ", " + node.getType ());
  ...
  public TNode lookupNameInCurrentScope(String name) {
    ...
    while (tnode == null && scope != null) {
      ...
      // System.out.println (">>> " + symTable.get ("u32"));
      scope = removeOneLevelScope(scope);
    }
    // System.err.println ("Returning: " + tnode);
    return tnode;
  }

  /*
   * Lookup an unscoped name in the table by prepending the current scope. If
   * not found, pops scope and looks again. Returns the empty String if all
   * scopes have been popped and nothing was found. Returns the fully scoped
   * name of the found table entry otherwise.
   */
  public String lookupScopedNameInCurrentScope (String name)
  {
    String scope = currentScopeAsString ();
    String scopedName;
    ...
    {
      scopedName = addScopeToName (scope, name);
      if (symTable.get (scopedName) != null)
        return scopedName;
      scope = removeOneLevelScope (scope);  
    }
