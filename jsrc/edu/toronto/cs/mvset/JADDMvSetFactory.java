...
public class JADDMvSetFactory extends AbstractMvSetFactory
{
  ...
  public MvSet createPoint (AlgebraValue[] args, AlgebraValue value)
  {
	...
	if (args [i] == noValue)
	  ...
	  {
	    int[] enum = enumType.bitValue (args [i]);
  ...
  public MvSet var (int argIdx, AlgebraValue argVal, AlgebraValue value)
  {
    ...
    int[] enum = enumType.bitValue (argVal);
    for (int i = 0; i < enum.length; i++)
      {
	if (enum [i] == -1)
	  ddArgIdx [argIdx + i] = MDDManager.NO_VALUE;
	else
	  ddArgIdx [argIdx + i] = enum [i];
      }
  ...
  {
    int[] enum = enumType.bitValue (getAlgebra ().getValue (branch));
  ...
  public class JADDMvSet extends AbstractMvSet 
  {
    ...
    {
      int[] enum = enumType.bitValue (value);
      
      MDDNode result = node;      
      for (int i = 0; i < enum.length; i++)
	{
	  if (enum [i] == -1) continue;
	  mddManager.cofactor (result, argIdx + i, enum [i]);
	}
